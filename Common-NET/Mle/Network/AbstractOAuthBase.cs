...
namespace Mle.Network {
    /// <summary>
    /// Ripped and slightly adapted from http://code.msdn.microsoft.com/Extending-HttpClient-with-8739f267
    /// </summary>
    public abstract class AbstractOAuthBase {
        /// <summary>
        ...
        protected abstract string GenerateSignatureUsingHash(string signatureBase, byte[] key);
        /// <summary>
        ...
        public string GenerateSignature(Uri url, string consumerKey, string consumerSecret, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce, SignatureTypes signatureType, out string normalizedUrl, out string normalizedRequestParameters, out string authHeader) {
            ...
            switch(signatureType) {
                ...
                case SignatureTypes.HMACSHA1:
                    ...
                    //hmacsha1.Key = Strings.StringToAscii(string.Format("{0}&{1}", UrlEncode(consumerSecret), string.IsNullOrEmpty(tokenSecret) ? "" : UrlEncode(tokenSecret)));
                    //string signature = GenerateSignatureUsingHash(signatureBase, hmacsha1);
                    
                    var key = Strings.AsciiStringToBytes(string.Format("{0}&{1}", UrlEncode(consumerSecret), string.IsNullOrEmpty(tokenSecret) ? "" : UrlEncode(tokenSecret)));
                    string signature = GenerateSignatureUsingHash(signatureBase, key);
